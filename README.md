# Ganesh Mirashi - Portfolio

Full Stack Developer with 2 years of experience in Next.js and Django REST Framework.

## 🚀 About Me
I am currently working at GTPL, a startup in Bangalore, building scalable fintech applications including lending and KYC platforms.

## 🛠️ Tech Stack
- Next.js
- React
- Django REST Framework
- PostgreSQL
- Kafka
- TypeScript

## 📂 Projects

### MvLEND
Digital lending platform handling loan processing and transactions using scalable APIs and event-driven architecture.

### MvKYC
KYC verification platform with role-based access and blockchain-based identity validation.

## 📫 Contact
- Email: ganeshmirashi62@gmail.com
- LinkedIn: https://linkedin.com/in/yourprofile